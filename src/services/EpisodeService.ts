...
export class EpisodeService extends Service {
  ...
  public async create(episode: Episode): Promise<number> {
    ...
    console.log(episode)
    try {
      const res = await client.query(sql, [episode.episode_no, episode.episode_name, episode.info, episode.trailer_url, episode.image_url, episode.season_no, episode.show_name]);
  ...
  public async createEpisodeComment(sname: string, season: number, eid: number, comment: Comment): Promise<number> {
    ...
    const client = await this.pool.connect();
    const sql = 'INSERT INTO episode_comment (show_name, season_no, episode_no, comment_id) VALUES($1, $2, $3, $4) RETURNING comment_id';
    try {
  ...
  public async findEpisodeComments(sname: string, season: number, episode:number): Promise<Comment[]> {
    const client = await this.pool.connect();
    const sql =
      'SELECT comment_id, users.image_url, comment_body, users.id, users.username, subcomment_count, created_at FROM episode_comment ' +
      'INNER JOIN comment ON episode_comment.comment_id = comment.id ' +
      'INNER JOIN users ON comment.user_id = users.id WHERE show_name = $1 AND season_no = $2 AND episode_no = $3 ORDER BY comment.created_at DESC';
  ...
  public async findEpisodeComment(sname: string, season: number, eid: number, cid: number): Promise<Comment> {
    ...
    'SELECT comment_id, users.image_url, comment_body, users.id, users.username, subcomment_count, created_at FROM episode_comment ' +
    'INNER JOIN comment ON episode_comment.comment_id = comment.id ' +
    'INNER JOIN users ON comment.user_id = users.id WHERE show_name = $1 AND season_no = $2 AND episode_no = $3 AND comment_id = $4';
  ...
  public async markAsWatched(sname: string, season: number, episode: number, uid: number) {
    ...
    const client = await this.pool.connect();
    const sql = 'INSERT INTO episode_watch(watch_id, show_name, season_no, episode_no) VALUES($1, $2, $3, $4) RETURNING watch_id';
  ...
  public async unmarkWatch(sname: string, season: number, episode: number, uid: number): Promise<Boolean> {
    const client = await this.pool.connect();
    const sql = 'DELETE FROM watch WHERE id IN (SELECT watch_id FROM episode_watch WHERE show_name = $1 AND season_no = $2 AND episode_no = $3) AND user_id = $4';
    try {
