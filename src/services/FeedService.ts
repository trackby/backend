...
export class FeedService extends Service {
  public async findEpisodeComments(uid: number) {
    ...
    const sql = `SELECT episode.show_name, episode.season_no, episode.episode_no, comment.comment_body,
                comment.created_at, \'episodecomment\'
                AS type
  ...
  public async findSubcomments(uid: number): Promise<Comment[]> {
    const client = await this.pool.connect();
    const sql = `SELECT parent.comment_body as parent_body, users.username, users.image_url, sub.comment_body,
                sub.subcomment_count, sub.created_at, \'subcomment\'
                AS type
  ...
  public async findSeasonComments(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, users.image_url, season.show_name, season.season_no, comment.comment_body,
                comment.created_at, \'seasoncomment\' AS type FROM season_comment
  ...
  public async findShowComments(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, users.image_url, tvshow.show_name, comment.comment_body, comment.created_at,
                \'showcomment\' as type FROM show_comment
  ...
  public async findSeasonWatches(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, season.show_name, season.season_no, watch.created_at,
                  \'seasonwatches\' as type FROM season_watch
  ...
  public async findEpisodeWatches(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, episode.show_name, episode.season_no, episode.episode_no, watch.created_at,
                \'episodewatches\' as type FROM episode_watch
  ...
  public async findShowRates(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, tvshow.show_name, rate.rating, rate.created_at, \'showrates\' as type
                 FROM show_rate
  ...
  public async findSeasonRates(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, season.show_name, season.season_no, rate.rating, rate.created_at,
                \'seasonrates\' as type  FROM season_rate
  ...
  public async findEpisodeRates(uid: number) {
    const client = await this.pool.connect();
    const sql = `SELECT users.username, episode.show_name, episode.season_no, episode.episode_no, rate.rating,
                rate.created_at, \'episoderates\' as type  FROM episode_rate
                INNER JOIN episode ON episode_rate.show_name = episode_rate.show_name
                AND episode_rate.season_no = episode.season_no AND
